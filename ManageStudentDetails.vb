...
Public Class ManageStudentDetails
    ...
    Private Sub SearchTextBox_LostFocus(sender As Object, e As EventArgs) Handles SearchTextBox.LostFocus
        ...
        End If
    End Sub
    Public Sub ManageStudentDetails_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ConnectionTimer.Enabled = True
        TimeTimer.Enabled = True
        SearchTextBox.ForeColor = Color.Gray
        SearchTextBox.Text = "Search"
        SearchComboBox.SelectedIndex = 0
        ActiveControl = ManageStudentDetailsPanel
        ManageStudentDetailsStatusLabel.Text = ""
        ManageStudentDetailsStatusLabel.ForeColor = Color.Red
        ...
        Try
            MysqlConn.Open()
            MySqlComm = New MySqlCommand("select student_id as 'ID', concat_ws(' ', ' ', first_name, middle_name, last_name) as 'Name', gender as 'Gender', email_id as 'Email ID', mobile_no as 'Mobile No.', concat_ws(', ', city, country) as 'Location' from ams.student", MysqlConn)
            SDA.SelectCommand = MySqlComm
            SDA.Fill(DataTab)
            StudentDetailsDataGridView.DataSource = DataTab
            MysqlConn.Close()
        ...
        Finally
            MysqlConn.Dispose()
        End Try
        StudentDetailsDataGridView.Columns(0).Visible = False
        StudentDetailsDataGridView.Sort(StudentDetailsDataGridView.Columns(1), ComponentModel.ListSortDirection.Ascending)
        StudentDetailsDataGridView.ClearSelection()
        ID = Nothing
    End Sub
    Private Sub StudentDetailsDataGridView_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles StudentDetailsDataGridView.CellClick
        If e.RowIndex >= 0 Then
            Dim Row As DataGridViewRow
            Row = StudentDetailsDataGridView.Rows(e.RowIndex)
    ...
    Private Sub SearchTextBox_TextChanged(sender As Object, e As EventArgs) Handles SearchTextBox.TextChanged
        ...
        Try
            If SearchTextBox.Text <> "Search" And SearchTextBox.ForeColor <> Color.Gray And SearchComboBox.SelectedIndex = 0 Then
                ...
                Dim DataTab As New DataTable
                MysqlConn.Open()
                MySqlComm = New MySqlCommand("select student_id as 'ID', concat_ws(' ', ' ', first_name, middle_name, last_name) as 'Name', gender as 'Gender', email_id as 'Email ID', mobile_no as 'Mobile No.', concat_ws(', ', city, country) as 'Location' from ams.student where (first_name like concat('%',@searchstring,'%') or middle_name like concat('%',@searchstring,'%') or last_name like concat('%',@searchstring,'%'))", MysqlConn)
            ...
            ElseIf SearchTextBox.Text <> "Search" And SearchTextBox.ForeColor <> Color.Gray And SearchComboBox.SelectedIndex = 1 Then
                ...
                Dim DataTab As New DataTable
                MysqlConn.Open()
                MySqlComm = New MySqlCommand("select student_id as 'ID', concat_ws(' ', ' ', first_name, middle_name, last_name) as 'Name', gender as 'Gender', email_id as 'Email ID', mobile_no as 'Mobile No.', concat_ws(', ', city, country) as 'Location' from ams.student where email_id like concat('%',@searchstring,'%')", MysqlConn)
                MySqlComm.Parameters.AddWithValue("@searchstring", value:=SearchTextBox.Text)
                SDA.SelectCommand = MySqlComm
                SDA.Fill(DataTab)
                StudentDetailsDataGridView.DataSource = DataTab
            ...
            ElseIf SearchTextBox.Text <> "Search" And SearchTextBox.ForeColor <> Color.Gray And SearchComboBox.SelectedIndex = 2 Then
                ...
                Dim DataTab As New DataTable
                MysqlConn.Open()
                MySqlComm = New MySqlCommand("select student_id as 'ID', concat_ws(' ', ' ', first_name, middle_name, last_name) as 'Name', gender as 'Gender', email_id as 'Email ID', mobile_no as 'Mobile No.', concat_ws(', ', city, country) as 'Location' from ams.student where mobile_no like concat('%',@searchstring,'%')", MysqlConn)
                MySqlComm.Parameters.AddWithValue("@searchstring", value:=SearchTextBox.Text)
                SDA.SelectCommand = MySqlComm
                SDA.Fill(DataTab)
                StudentDetailsDataGridView.DataSource = DataTab
            ...
            ElseIf SearchTextBox.Text <> "Search" And SearchTextBox.ForeColor <> Color.Gray And SearchComboBox.SelectedIndex = 3 Then
                ...
                Dim DataTab As New DataTable
                MysqlConn.Open()
                MySqlComm = New MySqlCommand("select student_id as 'ID', concat_ws(' ', ' ', first_name, middle_name, last_name) as 'Name', gender as 'Gender', email_id as 'Email ID', mobile_no as 'Mobile No.', concat_ws(', ', city, country) as 'Location' from ams.student where (city like concat('%',@searchstring,'%') or country like concat('%',@searchstring,'%'))", MysqlConn)
                MySqlComm.Parameters.AddWithValue("@searchstring", value:=SearchTextBox.Text)
                SDA.SelectCommand = MySqlComm
                SDA.Fill(DataTab)
                StudentDetailsDataGridView.DataSource = DataTab
    ...
    Private Sub ViewButton_Click(sender As Object, e As EventArgs) Handles ViewButton.Click
        If ID = Nothing Then
            ManageStudentDetailsStatusLabel.Text = "Please select a student record first!"
        Else
            ViewStudentDetails.ViewStudentDetails_Load(e, e)
