...
Public Class AddUpdateStaffDetails
    ...
    Private Sub ValidationTimer_Tick(sender As Object, e As EventArgs) Handles ValidationTimer.Tick
        If Home.StaffDetails = False Then
            MysqlConn = New MySqlConnection With {.ConnectionString = "server = localhost; userid = root; password = root; database = ams;"}
            ...
            MysqlConn.Open()
            MySqlComm = New MySqlCommand("select * from ams.staff where binary username = @username and staff_id != @userid", MysqlConn)
        ...
        Else
            ...
            EmailIDExists = False
            MysqlConn.Open()
            MySqlComm = New MySqlCommand("select * from ams.staff where email_id = @emailid", MysqlConn)
            MySqlComm.Parameters.AddWithValue("@emailid", value:=EmailIDTextBox.Text)
            MySqlRead = MySqlComm.ExecuteReader
            While MySqlRead.Read
                EmailIDExists = True
        ...
        End If
        If ActiveControl Is UsernameTextBox And UsernameTextBox.Text = "" Then
            UsernameStatusLabel.Text = "Please enter your Username!"
        ElseIf ActiveControl Is UsernameTextBox And UsernameExists = True Then
            UsernameStatusLabel.Text = "Username already exists!"
        ElseIf ActiveControl Is UsernameTextBox And UsernameTextBox.Text <> "" Then
            UsernameStatusLabel.Text = ""
        End If
        If ActiveControl Is PasswordTextBox And PasswordTextBox.Text = "" Then
            PasswordStatusLabel.Text = "Enter your New Password!"
        ElseIf ActiveControl Is PasswordTextBox And Script.CheckPassword(PasswordTextBox.Text) = 3 Then
            PasswordStatusLabel.Text = "Allowed Characters are A-Z, a-z, 0-9!"
        ElseIf ActiveControl Is PasswordTextBox And Script.CheckPassword(PasswordTextBox.Text) = 2 Then
            PasswordStatusLabel.Text = "New Password must atleast contain 1 U-Case letter, 1 L-Case letter and 1 number!"
        ElseIf ActiveControl Is PasswordTextBox And Script.CheckPassword(PasswordTextBox.Text) <> 3 And PasswordTextBox.Text.Length() < 8 Or PasswordTextBox.Text.Length() > 16 Then
            PasswordStatusLabel.Text = "New Password must be betwwen 8-16 letters!"
        ElseIf ActiveControl Is PasswordTextBox And PasswordTextBox.Text <> "" And Script.CheckPassword(PasswordTextBox.Text) = 1 And PasswordTextBox.Text.Length() >= 8 And PasswordTextBox.Text.Length() <= 16 Then
    ...
    End Sub
    Public Sub AddUpdateStaffDetails_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ConnectionTimer.Enabled = True
        TimeTimer.Enabled = True
        ValidationTimer.Enabled = True
        ActiveControl = AddUpdateStaffDetailsPanel
        AddUpdateStaffDetailsStatusLabel.Text = "Fields marked with * are mandatory!"
        AddUpdateStaffDetailsStatusLabel.ForeColor = Color.Red
            ...
            Try
                ...
                While MySqlRead.Read
                    ...
                    If Not IsDBNull(MySqlRead("username")) Then
                        UsernameTextBox.ForeColor = Color.Black
                        UsernameTextBox.Text = MySqlRead.GetString("username")
                    Else
                        UsernameTextBox.ForeColor = Color.Gray
                        UsernameTextBox.Text = "Username"
                    End If
                    If Not IsDBNull(MySqlRead("password")) Then
                        PasswordTextBox.ForeColor = Color.Black
                        PasswordTextBox.Text = MySqlRead.GetString("password")
                    Else
                        PasswordTextBox.ForeColor = Color.Gray
                        PasswordTextBox.Text = "Password"
                    End If
    ...
    Private Sub AddButton_Click(sender As Object, e As EventArgs) Handles AddButton.Click
        ...
        End If
        If (FirstNameTextBox.Text <> "" Or FirstNameTextBox.ForeColor <> Color.Gray) And (LastNameTextBox.Text <> "" Or LastNameTextBox.ForeColor <> Color.Gray) And BirthDatePicker.Value.Date < Date.Now.Date And GenderValue <> "" And (EmailIDTextBox.Text <> "" Or EmailIDTextBox.ForeColor <> Color.Gray) And InvalidEmail = False And EmailIDExists = False And (MobileNoTextBox.Text <> "" Or MobileNoTextBox.ForeColor <> Color.Gray) And IsNumeric(MobileNoTextBox.Text) = True And MobileNoTextBox.Text.Length() = 10 And (AddressTextBox.Text <> "" Or AddressTextBox.ForeColor <> Color.Gray) And (ZIPCodeTextBox.Text <> "" Or ZIPCodeTextBox.ForeColor <> Color.Gray) And (CityTextBox.Text <> "" Or CityTextBox.ForeColor <> Color.Gray) And (ProvinceTextBox.Text <> "" Or ProvinceTextBox.ForeColor <> Color.Gray) And (CountryTextBox.Text <> "" Or CountryTextBox.ForeColor <> Color.Gray) And (UsernameTextBox.Text <> "" Or UsernameTextBox.ForeColor <> Color.Gray) And UsernameExists = False And (PasswordTextBox.Text <> "" Or PasswordTextBox.ForeColor <> Color.Gray) And Script.CheckPassword(PasswordTextBox.Text) = 1 Then
            Try
                MysqlConn.Open()
                MySqlComm = New MySqlCommand("insert into ams.staff set first_name = @firstname,middle_name = @middlename,last_name = @lastname,dob = @birthdate,gender = @gender,email_id = @emailid,mobile_no = @mobileno,address = @address,zip_code = @zipcode,city = @city,province = @province,country = @country,username = @username,password = @password,profile_picture = @profilepicture", MysqlConn)
                ...
                MySqlRead = MySqlComm.ExecuteReader
                AddUpdateStaffDetailsStatusLabel.ForeColor = Color.Green
                AddUpdateStaffDetailsStatusLabel.Text = "Staff Added!"
                MysqlConn.Close()
            Catch ex As MySqlException
                ErrorBox.ErrorBox_Load(e, e)
                ErrorBox.StatusLabel.Text = ex.Message
        ...
        Else
            ...
            End If
            If PasswordTextBox.Text = "Password" And PasswordTextBox.ForeColor = Color.Gray Then
                PasswordStatusLabel.Text = "Please enter your Password!"
            End If
        End If
    End Sub
    Private Sub UpdateButton_Click(sender As Object, e As EventArgs) Handles UpdateButton.Click
        ...
        If (FirstNameTextBox.Text <> "" Or FirstNameTextBox.ForeColor <> Color.Gray) And (LastNameTextBox.Text <> "" Or LastNameTextBox.ForeColor <> Color.Gray) And BirthDatePicker.Value.Date < Date.Now.Date And GenderValue <> "" And (EmailIDTextBox.Text <> "" Or EmailIDTextBox.ForeColor <> Color.Gray) And InvalidEmail = False And EmailIDExists = False And (MobileNoTextBox.Text <> "" Or MobileNoTextBox.ForeColor <> Color.Gray) And IsNumeric(MobileNoTextBox.Text) = True And MobileNoTextBox.Text.Length() = 10 And (AddressTextBox.Text <> "" Or AddressTextBox.ForeColor <> Color.Gray) And (ZIPCodeTextBox.Text <> "" Or ZIPCodeTextBox.ForeColor <> Color.Gray) And (CityTextBox.Text <> "" Or CityTextBox.ForeColor <> Color.Gray) And (ProvinceTextBox.Text <> "" Or ProvinceTextBox.ForeColor <> Color.Gray) And (CountryTextBox.Text <> "" Or CountryTextBox.ForeColor <> Color.Gray) And (UsernameTextBox.Text <> "" Or UsernameTextBox.ForeColor <> Color.Gray) And UsernameExists = False And (PasswordTextBox.Text <> "" Or PasswordTextBox.ForeColor <> Color.Gray) And Script.CheckPassword(PasswordTextBox.Text) = 1 Then
            Try
                ...
                MySqlRead = MySqlComm.ExecuteReader
                AddUpdateStaffDetailsStatusLabel.ForeColor = Color.Green
                AddUpdateStaffDetailsStatusLabel.Text = "Profile Updated!"
                MysqlConn.Close()
            Catch ex As MySqlException
                ErrorBox.ErrorBox_Load(e, e)
                ErrorBox.StatusLabel.Text = ex.Message
        ...
        Else
            ...
            If PasswordTextBox.Text = "Password" And PasswordTextBox.ForeColor = Color.Gray Then
                PasswordStatusLabel.Text = "Please enter your Password!"
            ElseIf Script.CheckPassword(PasswordTextBox.Text) = 3 Then
                PasswordStatusLabel.Text = "Allowed Characters are A-Z, a-z, 0-9!"
            ElseIf Script.CheckPassword(PasswordTextBox.Text) = 2 Then
                PasswordStatusLabel.Text = "New Password must atleast contain 1 U-Case letter, 1 L-Case letter and 1 number!"
            ElseIf Script.CheckPassword(PasswordTextBox.Text) <> 3 And (PasswordTextBox.Text.Length() < 8 Or PasswordTextBox.Text.Length() > 16) Then
                PasswordStatusLabel.Text = "New Password must be betwwen 8-16 letters!"
            End If
        End If
    End Sub
